...
class ModelTraitNormalVB(object):
    ...
    def update(self, itr):
        ...
        self.beta_mean, self.beta_var, self.tau_mean, self.tau_var = \
            _update_beta_tau(self.YTY, self.GTG, self.GTY, self.G, self.zeta_mean)
...
def _update_beta_tau(YTY, GTG, GTY, G, zeta_mean):
    ...
    A = GTG + _nmp.diag(zeta_mean)
    beta_mean = _utils.chol_solve(A, GTY)
    beta_var = rate / (shape - 1) * _nmp.diag(_nmp.linalg.inv(A))

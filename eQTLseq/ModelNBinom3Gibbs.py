...
class ModelNBinom3Gibbs(object):
    ...
    def update(self, itr, **args):
        ...
        Z = Z[:, idxs_genes]
        G = G[:, idxs_markers]
        mu = self.mu[idxs_genes]
        phi = self.phi[idxs_genes]
        beta = self.beta[idxs_genes, :][:, idxs_markers]
        zeta = self.zeta[idxs_genes, :][:, idxs_markers]
        eta = self.eta[idxs_markers]
        tau = self.tau[idxs_genes]

        # update beta
        self.beta[_nmp.ix_(idxs_genes, idxs_markers)] = _sample_beta(Z, G, mu, phi, beta, tau, zeta, eta)
...
def _sample_beta(Z, G, mu, phi, beta, tau, zeta, eta):
    """TODO."""
    _, n_markers = G.shape

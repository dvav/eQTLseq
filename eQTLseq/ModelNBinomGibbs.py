...
class ModelNBinomGibbs(_ModelNormalGibbs):
    ...
    def update(self, itr, **args):
        ...
        GTY = G.T.dot(self.Y)
        super().update(itr, YTY=YTY, GTY=GTY, phi=self.phi, **args)
...
def _sample_phi_global(Z, G, mu, phi, Y, beta, tau, zeta, eta, mu_phi, tau_phi):
    n_samples, n_genes = Z.shape
    means = mu * _nmp.exp(Y)
    GBT = G.dot(beta.T)
    resid2 = (Y - GBT)**2
...
def _sample_phi_local(Z, G, mu, phi, Y, beta, tau, zeta, eta, mu_phi, tau_phi, scale=0.01):
    n_samples, n_genes = Z.shape
    means = mu * _nmp.exp(Y)
    GBT = G.dot(beta.T)
    resid2 = (Y - GBT)**2
...
def _sample_phi(Z, G, mu, phi, Y, beta, tau, zeta, eta, mu_phi, tau_phi):
    if _rnd.rand() < 0.5:
        phi = _sample_phi_local(Z, G, mu, phi, Y, beta, tau, zeta, eta, mu_phi, tau_phi)
    else:
        phi = _sample_phi_global(Z, G, mu, phi, Y, beta, tau, zeta, eta, mu_phi, tau_phi)

...
class ModelNBinomGibbs3(object):
    ...
    def update(self, itr, **args):
        ...
        YTY = _nmp.sum(Y**2, 0)
        GTY = G.T.dot(Y)
        beta, tau = _sample_beta_tau(YTY, GTG, GTY, zeta, eta, phi, args['n_samples'], s2_lims)

        self.beta[_nmp.ix_(self.idxs_genes, self.idxs_markers)] = beta
        self.tau[self.idxs_genes] = tau

        # sample eta and zeta
        self.zeta = _sample_zeta(self.beta, self.tau, self.eta, self.phi)
        self.zeta = _nmp.clip(self.zeta, 1 / s2_lims[1], 1 / s2_lims[0])

        self.eta = _sample_eta(self.beta, self.tau, self.zeta, self.phi)
...
def _sample_phi_local(Z, G, c, mu, phi, Y, beta, tau, zeta, eta, mu_phi, tau_phi, scale=0.01):
    ...
    loglik = (_spc.gammaln(Z + alpha) - _spc.gammaln(alpha) + alpha * _nmp.log(pi) + Z * _nmp.log1p(-pi)).sum(0) \
        ...
        - 0.5 * (log_phi[:, None] + zeta * eta * theta[:, None] * beta**2).sum(1)
...
def _sample_phi(Z, G, norm_factors, mu, phi, Y, beta, tau, zeta, eta, mu_phi, tau_phi):
    """TODO."""
    # if _rnd.rand() < 0.5:
    #     phi = _sample_phi_local(Z, G, norm_factors, mu, phi, Y, beta, tau, zeta, eta, mu_phi, tau_phi)
    # else:
    phi = _sample_phi_global(Z, G, norm_factors, mu, phi, Y, beta, tau, zeta, eta, mu_phi, tau_phi)
...
def _sample_beta_tau(YTY, GTG, GTY, zeta, eta, phi, n_samples, s2_lims):
    """TODO."""

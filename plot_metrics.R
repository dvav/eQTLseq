...
metrics_flat = metrics %>%
  ...
  mutate(MODEL = ifelse(MODEL %in% c('NBinomial', 'Binomial', 'Poisson'), MODEL, TRANS)) %>%
...
metrics_filt = metrics_flat %>%
  filter(METRIC %in% c(metric)) %>%
  group_by(NSAMPLES, NMARKERS, NGENES, MODEL, METRIC) %>%
  arrange(desc(VALUE)) %>%
  slice(1:3) %>%
  ungroup() %>%
